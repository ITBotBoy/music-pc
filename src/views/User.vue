    ...
    <div class="user-page-container">
            ...
            <div class="login-input-container">
                <template v-if="$route.query.setcookie">
                    <div class="input-line">
                        ...
                        <el-input type="textarea" v-model="editCookie"></el-input>
                    </div>
                    <div
                        @click="setCookie"
                        :class="`login-btn ${!editCookie && 'disabled'}`"
                    >设置cookie
                    </div>
                </template>
                ...
                <div class="input-line" v-if="type === 'captcha'">
                    ...
                    <div @click="sendCaptcha" :class="`code-btn ${time > 0 && 'disabled'}`">
                        发送{{time > 0 ? `（${time}s）` : ''}}
                    </div>
                </div>
                <div
        ...
        <div class="user-info-content" v-if="uInfo.userId">
            ...
            <div class="user-info-list">
                ...
                <div class="user-info-txt user-info-listen pointer" @click="selected = 'history'">听过
                    {{uInfo.listenSongs}}
                </div>
                <div class="user-info-txt user-info-playlist">
                    <router-link :to="`/playlist?id=${uInfo.userId}&from=163`">歌单 {{uInfo.playlistCount}} 个
                    </router-link>
                </div>
            ...
            <div class="user-right-list">
                ...
                <!-- 关注、粉丝 -->
                <div class="member-list" @scroll="onScroll('.user-page-container .member-list')"
                     v-if="selected === 'flow' || selected === 'fans'">
                    <div v-for="(u, i) in uInfo[`${selected}List`]" :key="`${u.userId}-${i}`" class="member-item">
                        <router-link :to="`/user?id=${u.userId}`">
                            <img class="member-img"
                                 :src="`${String(u.avatarUrl) === 'null' ? 'http://p3.music.126.net/VnZiScyynLG7atLIZ2YPkw==/18686200114669622.jpg' : u.avatarUrl}?param=120y120`"/>
                            <div class="member-name">{{u.nickname}}</div>
                        </router-link>
                    </div>
                    <div class="text-center mt_40"
                         v-if="uInfo[`${selected}List`] && uInfo[`${selected}List`].length === 0">空空如也～
                    </div>
                </div>
            </div>
        </div>
