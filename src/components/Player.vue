<template>
    <div class="player-container">
          ...
          <span>
            ...
            <div class="other-control inline-block">
                ...
                <div class="order-control" @mouseleave="showOrder = false">
                    <div v-if="showOrder" class="order-list-container" @mouseleave="showOrder = false"
                         @mouseover="showOrder = true">
                        <div class="order-list">
                            <div v-for="key in orderList" v-if="orderType !== key" :key="`order-${key}`"
            ...
            <span @click="playlistTracks(playNow.aId, 'add', 'ADD_SONG_2_LIST')">
              <i class="iconfont icon-add ft_16 pointer"/>
                ...
                <!--todo 待修改-->
                <input id="cp-share-input"
                       :value="changeUrlQuery({ shareId: playNow.id, from: playNow.platform, shareCid: playNow.cid }, 'http://music.renserve.com/', false)">
            ...
            <span>
              <i class="iconfont icon-list ft_16 pointer"/>
                ...
                <div class="more-control" @mouseleave="showMore = false">
                    <div v-if="showMore" class="more-list-container" @mouseleave="showMore = false"
                         @mouseover="showMore = true">
                        <div class="more-list">
                            <div v-for="more in moreList" @click="handleClickMore(more.key)"
                                 :key="`more-key-${more.key}`">
                ...
                <div class="back-container">
                    <router-link class="iconfont icon-feedback" to="/feedback"/>
        ...
        <!--  用来连接 audioContext  -->
        <audio :class="usingAudioContext && 'played-audio'" id="m-player" crossOrigin="anonymous"
               :src="usingAudioContext ? (playingUrl || '') : ''" controls/>
        
        <!--  原生的audio 播放，用来兼容 audio 不支持的情况  -->
        <audio v-if="!usingAudioContext" :class="!usingAudioContext && 'played-audio'" id="v-player" v-bind="vBind"
               :src="playingUrl || ''" controls/>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        usingAudioContext() {
            return this.openAudioCtx && this.canUseAudioCtx;
        }
    },
    watch: {
        async playNow(v) {
            ...
            if ('mediaSession' in navigator && window.MediaMetadata) {
                navigator.mediaSession.metadata = new window.MediaMetadata({
                    ...
                    artwork: [
                        {
                            src: al.picUrl || 'http://p2.music.126.net/ftPcA5oCeIQxhiNmEpmtKw==/109951163926974610.jpg',
                            sizes: '96x96'
                        },
                        {
                            src: al.picUrl || 'http://p2.music.126.net/ftPcA5oCeIQxhiNmEpmtKw==/109951163926974610.jpg',
            ...
            if ((pUrl !== this.playingUrl || !this.playingUrl) && url) {
                ...
                // 非企鹅音乐，可以使用audioContext
                this.canUseAudioCtx = nUrl.indexOf('qq.com') === -1;
                this.$nextTick(async () => {
                    window.pDom = document.getElementsByClassName('played-audio')[0];
                    this.playerDom = window.pDom;
                    this.audioBind();
    ...
    mounted() {
        ...
        this.playerDom = document.getElementsByClassName('played-audio')[0];
        ...
        if ((window.AudioContext || window.webkitAudioContext)) {
            ...
            const draw = () => {
                this.openAudioCtx = Storage.get('useAudioContext') !== '0';
                this.drawMusic && this.drawMusic.draw(this.usingAudioContext);
    ...
    methods: {
        clearOldAudio() {
            ...
            pDom.muted = true;
            pDom.onended = null;
            pDom.ontimeupdate = null;
            pDom.onerror = null;
            pDom.oncanplaythrough = null;
        },
        audioBind() {
            const dispatch = this.$store.dispatch;
            const {pDom} = window;
